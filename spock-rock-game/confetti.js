...
var confetti = {
  ...
  function getRandom() {
	...
	return crArray[0] / 255;

  }

  function resetParticle(particle, width, height) {
	particle.color =
	  colors[(getRandom() * colors.length) | 0] + (confetti.alpha + ")");
	particle.color2 =
	  colors[(getRandom() * colors.length) | 0] + (confetti.alpha + ")");
	particle.x = getRandom() * width;
	particle.y = getRandom() * height - height;
	particle.diameter = getRandom() * 10 + 5;
	particle.tilt = getRandom() * 10 - 10;
	particle.tiltAngleIncrement = getRandom() * 0.07 + 0.05;
	particle.tiltAngle = getRandom() * Math.PI;
	return particle;
  }
	...
	if (min) {
	  if (max) {
		if (min == max) count = particles.length + max;
		  ...
		  count = particles.length + ((getRandom() * (max - min) + min) | 0);
		}
